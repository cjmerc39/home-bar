...
(async () => {
  ...
  const lblBoxes = [...d.querySelectorAll('#galaxy .star')].map(g => {
    ...
    const wl = lbl.textContent.length*5.0 + 5;
  ...
  const litStars = [...d.querySelectorAll('#galaxy .star')].filter(g=>/\b(mk|lw)\b/.test(g.getAttribute('class')||''));
  assert(litStars.length>0 && litStars.every(g=>g.querySelector('.slabel')!==null), 'every lit star keeps its name ('+litStars.length+' labeled, none floating anonymous)');
  assert([...d.querySelectorAll('#galaxy .lead')].every(l=>l.parentNode.querySelector('.slabel')!==null), 'leader lines only ever accompany a label');
  ...
  assert(/(January|February|March|April|May|June|July|August|September|October|November|December)/.test(ccReq.context.time), 'the DATE rides along — the concierge can smell the season');
  assert(typeof ccReq.context.weather==='string' && /°/.test(ccReq.context.weather), 'the first message already carries the weather (awaited, not raced)');
  // house drinks lead the AI list so the relay cap can never cut them; notes travel with the ingredients
  w.eval('S.recipes.find(r=>r.name==="Gimlet").house=true; save();');
  const mkOrdered = JSON.parse(w.eval('JSON.stringify(makeableDrinkList())'));
  assert(mkOrdered[0].name==='Gimlet', 'house drinks lead the AI drink list');
  assert(mkOrdered.some(x=>x.name==='Whiskey Sour' && /lovely/.test(x.ingredients)), 'recipe notes ride with the ingredients (seasonal hints reach the model)');
  w.eval('S.recipes.find(r=>r.name==="Gimlet").house=false; save();');
